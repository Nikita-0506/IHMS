...
DATASET_PATH = BASE_DIR / 'ml_models' / 'disease_prediction' / 'dataset.csv'
...
def train_disease_model(dataset_path=None, model_path=None):
	source = Path(dataset_path) if dataset_path else DATASET_PATH
	target = Path(model_path) if model_path else MODEL_PATH

	if not source.exists():
		raise FileNotFoundError(f'Disease dataset not found: {source}')

	df = pd.read_csv(source)
	if df.empty:
		raise ValueError('Disease dataset is empty.')

	target_col = 'disease' if 'disease' in df.columns else 'predicted_disease'
	if target_col not in df.columns:
		raise ValueError('Disease dataset must include disease or predicted_disease column.')

	drop_cols = {
		target_col,
		'predicted_disease',
		'prediction_confidence',
		'disease_risk_percent',
		'prediction_id',
		'prediction_result',
		'ai_recommendation',
		'prediction_model',
	...
	if not feature_cols:
		raise ValueError('No usable feature columns found for disease training.')
